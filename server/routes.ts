...
export async function registerRoutes(app: Express): Promise<Express> {
  ...
  // Save rewrite to archive
  app.post("/api/save-rewrite", async (req: Request, res: Response) => {
    try {
      const { originalText, rewrittenText, instructions, provider, parentRewriteId, rewriteLevel, stats } = req.body;
      
      if (!originalText || !rewrittenText || !instructions || !provider) {
        return res.status(400).json({ error: "Missing required fields" });
      }
      
      const { db } = await import('./db');
      const { rewrites } = await import('@shared/schema');
      
      const [savedRewrite] = await db.insert(rewrites).values({
        originalText,
        rewrittenText,
        instructions,
        provider,
        parentRewriteId: parentRewriteId || null,
        rewriteLevel: rewriteLevel || 1,
        stats: stats || null,
      }).returning();
      
      return res.json({ success: true, rewrite: savedRewrite });
    } catch (error: any) {
      console.error("Error saving rewrite:", error);
      return res.status(500).json({ 
        error: true, 
        message: error.message || "Failed to save rewrite" 
      });
    }
  });

  // Get rewrite archive
  app.get("/api/rewrite-archive", async (req: Request, res: Response) => {
    try {
      const { db } = await import('./db');
      const { rewrites } = await import('@shared/schema');
      const { desc } = await import('drizzle-orm');
      
      const rewriteHistory = await db
        .select()
        .from(rewrites)
        .orderBy(desc(rewrites.createdAt))
        .limit(50);
      
      return res.json({ success: true, rewrites: rewriteHistory });
    } catch (error: any) {
      console.error("Error fetching rewrite archive:", error);
      return res.status(500).json({ 
        error: true, 
        message: error.message || "Failed to fetch rewrite archive" 
      });
    }
  });

  // Get rewrite by ID
  app.get("/api/rewrite/:id", async (req: Request, res: Response) => {
    try {
      const rewriteId = parseInt(req.params.id);
      if (isNaN(rewriteId)) {
        return res.status(400).json({ error: "Invalid rewrite ID" });
      }
      ...
      return res.json({ success: true, rewrite: rewrite[0] });
    } catch (error: any) {
      console.error("Error fetching rewrite:", error);
      return res.status(500).json({ 
        error: true, 
        message: error.message || "Failed to fetch rewrite" 
      });
    }
  });
