...
const ComparativeResults: React.FC<ComparativeResultsProps> = ({
  ...
  // Check if we have the full comparison structure or intelligence comparison structure
  const isIntelligenceComparison = comparison && !comparison.comparisonTable && (comparison as any).detailedAnalysis;
  
  // Safe access to comparison data
  const finalJudgment = comparison?.finalJudgment || (comparison as any)?.detailedAnalysis || (comparison as any)?.summary || "";
  const comparisonTable = comparison?.comparisonTable || [];
  const documentAStyle = comparison?.documentA?.style || [];
  const documentBStyle = comparison?.documentB?.style || [];
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-700">{finalJudgment}</p>
      </div>
      ...
      {(documentAStyle.length > 0 || documentBStyle.length > 0) && (
        <div className="mb-6">
          <h3 className="font-semibold text-gray-800 mb-3">Style and Structure</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-gray-50 p-4 rounded-md">
              <h4 className="font-medium text-gray-800 mb-2">Document A</h4>
              {documentAStyle.length > 0 ? (
                <ul className="list-disc list-inside text-gray-700 space-y-1">
                  {documentAStyle.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              ) : (
                <p className="text-gray-500 italic">No detailed style analysis available</p>
              )}
            </div>
            <div className="bg-gray-50 p-4 rounded-md">
              <h4 className="font-medium text-gray-800 mb-2">Document B</h4>
              {documentBStyle.length > 0 ? (
                <ul className="list-disc list-inside text-gray-700 space-y-1">
                  {documentBStyle.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              ) : (
